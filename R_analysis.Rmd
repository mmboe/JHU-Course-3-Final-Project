...
#First remove (temporarily) the Subject ID and Activity variables to create a new data set to rename all variables and call in Justvars.
```{r}
...
```
#Add descriptive activity names to rename the numbers corresponding to activities in the Activity column. But first change class to factor and then create a vector of the activities for input into big dataset (Testtrain1).
```{r}
...
names(finalmeanstd) <- gsub(x = names(finalmeanstd), pattern = "\\)", replacement = "")
```
#For each of the 30 subjects and for each of the 6 activities, calculate the mean of each unique measure of the mean() and std() variables. Save in a dataset called md
```{r}
md <- finalmeanstd %>%
